...
export function Hero() {
  ...
  return (
    <SectionWrapper tone="paper" className="pt-28 pb-8 text-center lg:pt-40 lg:pb-9">
      <p className="font-sans text-base text-ink">{copy.hero.eyebrow}</p>

      <h1 className="mx-auto mt-2 max-w-4xl text-balance font-display text-3xl font-semibold leading-[1.04] tracking-[-0.03em] text-ink">
        ...
        <span className="block overflow-hidden pb-1">
          ...
          >
            {copy.hero.headline}{" "}
            <Highlight active={headlineSettled}>{copy.hero.highlightedWord}</Highlight>
          </motion.span>
        </span>
      </h1>
      ...
      <motion.p
        ...
        className="mx-auto mt-6 max-w-[560px] text-lg leading-relaxed text-ink-soft"
      >
        {copy.hero.lead}
      </motion.p>
      ...
      <motion.div
        {...entrance}
        transition={{ ...entrance.transition, delay: reduceMotion ? 0 : 0.22 }}
        className="mt-4 flex justify-center"
      >
        <MarginNote>{copy.hero.marginNote}</MarginNote>
      </motion.div>
    </SectionWrapper>
  );
}
